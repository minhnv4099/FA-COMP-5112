...
class BlenderConnection:
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Connected to Blender Server at {self.host}:{self.port}")
            return True
        except Exception as e:
            logger.error(f"Failed to connect to Blender Server: {str(e)}")
            self.sock = None
            return False
    ...
    def send_command(self, command_type: str, params: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Sending command: {command_type!r} with params: {params}")
...
def get_viewport_screenshot(ctx: Context, max_size: int = 800) -> str:
    ...
    try:
        ...
        temp_path = os.path.join(temp_dir, "blender", f"screenshot_{os.getpid()}.png")
        ...
        return f"The screenshot is saved in {temp_path!r}"
...
def execute_blender_code(ctx: Context, code: str) -> str:
    ...
    Always use when need to run code
...
def generate_hyper3d_model_via_images(
    ...
    try:
        blender = get_blender_connection()
        result = blender.send_command("create_rget_hyper3d_statusodin_job", {
            "text_prompt": None,
            "images": images,
            "bbox_condition": _process_bbox(bbox_condition),
        })
...
@add_note_docstring("The original prompt of Blender MCP")
@mcp.prompt()
def asset_creation_strategy() -> str:
    ...
    You are a expert Python programmer specialized in writing Blender Python (bpy) scripts compatible with Blender API v4.0
    You mission is to generate script: create a new script using logic from previous scripts and relevant documents, combining the necessary parts to complete the given subtask.
    Use 'execute_blender_code' tool to execute code.
    ...
    - Optional: add HDRI environment lighting for realism.s
    
    Camera Setup:
    - Not setup cameras
    
    Render Engine:
    - Use ```bpy.context.scene.render.engine = 'BLENDER_WORKBENCH'```
...
@mcp.prompt(name="code_generating_system_prompt")
def code_generating() -> str:
    """Defines the system prompt for the coding agent when needs to generate script"""
    return """You are a expert Python programmer specialized in writing Blender Python (bpy) scripts compatible with Blender API v4.0
    You mission is to generate script: create a new script using logic from previous scripts and relevant documents, combining the necessary parts to complete the given subtask.
    Use 'execute_blender_code' tool to execute code.
    With following instructions:
    ...
    - Must be compatible with Blender Python API v4.0
...
@mcp.prompt(name="code_fixing_system_prompt")
def code_fixing():
    """Defines the system prompt for the coding agent when needs to fix the script"""
    return """You are a expert Python programmer specialized in fixing Python scripts compatible with Blender API v4. 
    Your mission is to fix error: correct errors in the current script using related documents, updating only where needed without deleting the recent script

    Note: Never delete or rewrite the whole script. Always edit, extend, or refine the existing code instead. Keep all previous logic unless it is proven wrong or redundant. 
    When fixing, only modify the minimal necessary parts and preserve all useful sections, comments, and structure. 
    """


@mcp.prompt(name="code_improving_system_prompt")
def code_improving():
    """Defines the system prompt for the coding agent when needs to improve the script"""
    return """You are a expert Python programmer specialized in fixing Python scripts compatible with Blender API v4.
    You mission is to apply solution: improve the existing script based on a given instruction and related documents, modifying it without full replacement and ensuring the new version is clearly better.
    """
