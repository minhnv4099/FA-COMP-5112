...
from ..base.agent import AgentAsNode, InputT, register
from ..base.tool import execute_script, write_script
from ..base.utils import DirectionRouter
from ..utils.exception import ScriptWithError
...
class CodingAgent(AgentAsNode, name='Coding', use_model=True):
    ...
    def __init__(
            ...
            human_generate_template: str = None,
            human_fix_template: str = None,
            human_improve_template: str = None,
        ...
        self.human_generate_script_template = self._prepare_generate_template(human_generate_template)
        self.human_fix_error_template = self._prepare_fix_template(human_fix_template)
        self.human_apply_improvements_template = self._prepare_improve_template(human_improve_template)
    ...
    def _prepare_system_prompt(self, template: str = None):
        if template is None:
            return SystemMessagePromptTemplate.from_template("""
               ...
               """)
        return load_prompt_template(template)

    def _prepare_generate_template(self, template: str = None):
        if template is None:
    ...
    def _prepare_fix_template(self, template: str = None):
        if template is None:
            return """
        Help me to write python code using below information:
            subtask: {subtask}
    ...
    def _prepare_improve_template(self, template: str = None):
        if template is None:
            return """
        Help me to write python code using below information:
            subtask: {subtask}
    ...
    @override
    def _prepare_chat_prompt(self, human_prompt):
    ...
    def _generate_script(self, state):
        chat_prompt = ChatPromptTemplate([
            self.system_prompt,
            self.human_generate_script_template,
        ])
    ...
    def _fix_error(self, state) -> str:
        chat_prompt = self._prepare_chat_prompt(self.human_fix_error_template)
